...
var sendVarifyMail = require('./mail_sen.js');
        ...
        if(result.length==1){
            ...
            db.query(sql,[fname,mname,lname,email,pass,dob,dor,gender,username],function(err,result){
                ...
                if(result.insertId>0){
                    ...
                    req.session.msg = "Account created, please check mail to varify email";
                    res.redirect('/');
                }
                else{
...
var upload_details = multer({storage:storage});
app.post('/tweet_submit',upload_details.single('tweet_img'),function(req,res){
    const {post}=req.body;
     var filename ="";
     var mimetype="";
     try{
    ...
    db.query(sql,[req.session.userid,post,ct,filename,mimetype],
        function(err,result){
            if(err)
            throw err;
        if(result.insertId>0)
...
app.get('/varifyemail', function(req,res){
    ...
    db.query(sql_update,[email], function(err, result){
        if(err)
            console.log(err);
        if(result.affectedRows == 1){
            req.session.msg = "Enter varified now you can login with your password and email";
            res.redirect('/');
        }
        else{
...
app.post('/changepassword', (req, res) => {
    ...
    db.query(
        'SELECT * FROM user WHERE username = ? AND password = ?',[username, oldPassword],(error, results) => {
...
app.get('/profileBanner', (req, res) => {
    db.query("select * from user where uid=?",[req.session.userid],function(err,result,fields){
        if(err)
            throw err;
        if(result.length==1){
            res.render('profileBanner',{msg:"",result:result});
        }
        else
            res.redirect('/');
    });
...
app.post('/uploadProfilePicture', upload_details.fields([{ name: 'headerPicture', maxCount: 1 }, { name: 'profilePicture', maxCount: 1 }]), function(req, res) {
